## This file is created for the model in torch.nn, and the model is trained for ball tracking.
...
class BallTrackerNet(nn.Module):
    """
    Deep network for ball tracking
    """
...
clf = BallTrackerNet().to('cuda')
opt = Adadelta(clf.parameters(), lr=1)
loss_fn = nn.CrossEntropyLoss() 

# set training data
# train_dataset = TrackNetDataset(input_height, input_width, batch_size)
train_dataset = pd.read_csv('labels_train.csv')
...
if __name__ == "__main__": 
    ...
    for epoch in range(epochs): # train for 10 epochs
        for i in range(1,n): 
            X = train_dataset.loc[i, ['path1', 'path2', 'path3', 'gt_path', 'x-coordinate', 'y-coordinate']]
            # X = np.array(X)
            y = train_dataset.loc[1, ['visibility']]
            # y = np.array(y)
            # print("X", X)
            # print("y", y)
            X = torch.tensor(X)
    ...
    # Use the following code for load model
